...
class LightningLibriSpeechDataModule(pl.LightningDataModule):
    ...
    def _download_datasets(self) -> None:
        ...
        for part in self.LIBRISPECCH_PARTS:
            ...
            tar = tarfile.open(f"{self.configs.dateset.dataset_path}/{part}.tar.gz", mode="r:gz")
        ...
        for part in self.LIBRISPECCH_PARTS[-3:]:  # train
            path = os.path.join(self.configs.dataset.dateset_path, "LibriSpeech", part)
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        audio_paths, trascripts = self._parse_manifest_file(self.configs.dataset.mainfest_file_path)
